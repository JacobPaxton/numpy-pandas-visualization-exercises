...
a2 = a ** 2
print("Mean:", np.mean(a2), ", Standard Deviation:", np.std(a2))

# A common statistical operation on a dataset is centering. 
# This means to adjust the data such that the mean of the data is 0. 
# This is done by subtracting the mean from each data point. 
# Center the data set. See this link for more on centering.
print("-----Center the data set.-----")
a_mean = np.mean(a)
print(a - a_mean)

# Calculate the z-score for each data point. Recall that the z-score is given by:
print("-----Calculate the z-score for each data point.-----")
print((a - a_mean) / np.std(a))

# Copy the setup and exercise directions from 
# More Numpy Practice into your numpy_exercises.py and 
# add your solutions.
...
sum_of_b = np.array(np.sum(b, axis=1))

# Exercise 2 - refactor the following to use numpy. 
min_of_b = min(b[0]) if min(b[0]) <= min(b[1]) else min(b[1]) 

min_of_b = np.min([b[0], b[1]])

# Exercise 3 - refactor the following maximum calculation to find the answer with numpy.
max_of_b = max(b[0]) if max(b[0]) >= max(b[1]) else max(b[1])

max_of_b = np.max([b[0], b[1]])
...
squares_of_b =  np.append(np.square(b[0]), np.square(b[1]))
print(squares_of_b)
...
print(np.product(c * np.transpose(c)))
...
num_uniques_in_d = len(np.unique(d))
...
# Awesome Bonus For much more practice with numpy, Go to
# https://github.com/rougier/numpy-100 
# and clone the repo down to your laptop. Next, go to 
# https://github.com/new 
# to make a new repo. Name it numpy-100, and follow the directions to 
